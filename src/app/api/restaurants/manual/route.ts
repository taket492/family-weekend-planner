...
export async function POST(request: NextRequest) {
  try {
    ...
    // 新規レストラン作成
    const newRestaurant = await prisma.spot.create({
      data: {
        name: body.name,
    ...
    return NextResponse.json({
      ...
      message: 'レストランが正常に登録されました'
    })

  } catch (error) {
    console.error('Manual restaurant creation error:', error)
    
    if (error instanceof Error && error.message.includes('Unique constraint')) {
      return NextResponse.json(
        { error: '同じレストランが既に存在します' },
        { status: 409 }
      )
    }
    
    return NextResponse.json(
      { error: 'レストランの登録に失敗しました' },
      { status: 500 }
    )
  }
}
